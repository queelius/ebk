...
def resolve_library_path(library_path: Optional[Path]) -> Path:
    ...
    if library_path is None:
        ...
        if config.library.default_path:
            ...
            console.print("[yellow]  book-memex config --library-path ~/my-library[/yellow]")
...
book_app.add_typer(review_app, name="review")  # Nested: book-memex book review ...
...
def main(
    ...
    """
    book-memex - eBook metadata management tool with SQLAlchemy + SQLite backend.
...
@app.command()
def about():
    """Display information about book-memex."""
    console.print("[bold cyan]book-memex - eBook Metadata Management Tool[/bold cyan]")
    console.print("")
    ...
    console.print("  book-memex lib init <path>          Initialize new library")
    console.print("  book-memex lib migrate              Run database migrations")
    console.print("  book-memex lib backup -o file.tar   Create library backup")
    console.print("  book-memex lib check                Check library integrity")
    console.print("")
    console.print("[bold]Query & Discovery:[/bold]")
    console.print("  book-memex query search <query>     Full-text search")
    console.print("  book-memex query list               List books with filtering")
    console.print("  book-memex query stats              Show library statistics")
    console.print("  book-memex query sql <query>        Raw SQL queries (SELECT only)")
    console.print("")
    console.print("[bold]Command Groups:[/bold]")
    console.print("  book-memex import <subcommand>      Import books (add, folder, calibre, isbn)")
    console.print("  book-memex export <subcommand>      Export library (json, csv, html, opds)")
    console.print("  book-memex book <subcommand>        Book operations (info, read, rate, tag, purge)")
    console.print("  book-memex note <subcommand>        Manage marginalia")
    console.print("  book-memex tag <subcommand>         Manage hierarchical tags")
    console.print("  book-memex queue <subcommand>       Manage reading queue")
    console.print("  book-memex view <subcommand>        Manage views (named library subsets)")
    console.print("")
    console.print("[bold]Interactive:[/bold]")
    console.print("  book-memex serve                    Start web server")
    console.print("")
    console.print("[bold]Getting Started:[/bold]")
    console.print("  1. Initialize: book-memex lib init ~/my-library")
    console.print("  2. Import: book-memex import add book.pdf")
    console.print("  3. Search: book-memex query search 'python'")
    console.print("")
...
def init(
    ...
    Example:
        book-memex lib init ~/my-library
    """
    ...
    try:
        ...
        console.print(f"  Use 'book-memex import' to add books")
...
def migrate(
    ...
    Examples:
        book-memex lib migrate                    # Migrate default library
        book-memex lib migrate ~/my-library       # Migrate specific library
        book-memex lib migrate --check            # Check pending migrations
        book-memex lib migrate --version          # Show schema version
    """
    ...
    if not db_path.exists():
        ...
        console.print("Use 'book-memex init' to create a new library.")
        raise typer.Exit(code=1)
...
def backup(
    ...
    Examples:
        book-memex lib backup -o library-backup.tar.gz
        book-memex lib backup -o backup.zip --db-only           # Database only (small)
        book-memex lib backup ~/my-library -o full-backup.tar.gz
    """
    import tarfile
    import zipfile
    import shutil
...
def restore(
    ...
    Examples:
        book-memex lib restore backup.tar.gz ~/restored-library
        book-memex lib restore backup.zip ~/my-library --force   # Overwrite existing
    """
    import tarfile
    import zipfile
    import shutil
...
def import_add(
    ...
    Examples:
        book-memex import add book.pdf                    # Uses config default
        book-memex import add book.pdf ~/my-library
        book-memex import add book.epub --title "My Book" --authors "Author Name"
    """
    from .library_db import Library
...
def import_calibre(
    ...
    library_path: Path = typer.Argument(..., help="Path to book-memex library"),
    ...
    Examples:
        book-memex import calibre ~/Calibre/Library ~/my-library
        book-memex import calibre ~/Calibre/Library ~/my-library --limit 100
    """
    from .library_db import Library
    ...
    if not library_path.exists():
        console.print(f"[red]Error: Library not found: {library_path}[/red]")
        console.print(f"[yellow]Initialize a library first with: book-memex init {library_path}[/yellow]")
        raise typer.Exit(code=1)
...
def import_folder(
    ...
    library_path: Optional[Path] = typer.Argument(None, help="Path to book-memex library (uses config default if not specified)"),
    ...
    Examples:
        book-memex import folder ~/Downloads/Books ~/my-library
        book-memex import folder ~/Books ~/my-library --no-recursive
        book-memex import folder ~/Books ~/my-library --extensions pdf,epub --limit 100
        book-memex import folder ~/Books ~/my-library --dry-run      # Preview only
        book-memex import folder ~/Books ~/my-library --log-failures failed.txt
    """
    from .library_db import Library
    from .extract_metadata import extract_metadata
    import hashlib
...
def import_url(
    ...
    Examples:
        book-memex import url https://example.com/book.epub
        book-memex import url https://example.com/book.pdf ~/my-library
    """
    import httpx
    import re
    import tempfile
...
def import_opds(
    ...
    Examples:
        book-memex import opds https://example.com/opds/catalog.xml
        book-memex import opds https://library.example.com/opds --limit 50
    """
    import httpx
...
def import_isbn(
    ...
    Examples:
        book-memex import isbn 978-0-13-468599-1
        book-memex import isbn 0134685997 ~/my-library
    """
    import httpx
    import re
    from .library_db import Library
...
def search(
    ...
    Examples:
        book-memex query search "python programming"                  # Uses config default
        book-memex query search "python programming" ~/my-library
        book-memex query search "machine learning" --limit 50
        book-memex query search "python" --offset 20 --limit 20       # Page 2
    """
    from .library_db import Library
...
def stats(
    ...
    Example:
        book-memex query stats                # Uses config default
        book-memex query stats ~/my-library
    """
    from .library_db import Library
...
def sql(
    ...
    Examples:
        book-memex query sql "SELECT title, language FROM books WHERE language = 'en'"
        book-memex query sql "SELECT COUNT(*) as count FROM books" --format json
        book-memex query sql "SELECT a.name, COUNT(*) as books FROM authors a JOIN book_authors ba ON a.id = ba.author_id GROUP BY a.name ORDER BY books DESC" --limit 10
        book-memex query sql "SELECT * FROM books_fts WHERE books_fts MATCH 'python'" --format csv
    """
    import io
    import sqlite3
    ...
    if not query_stripped.startswith('SELECT'):
        ...
        console.print("[yellow]Use other book-memex commands to modify data[/yellow]")
        raise typer.Exit(code=1)
...
def list_books(
    ...
    Examples:
        book-memex query list                           # Uses config default
        book-memex query list ~/my-library
        book-memex query list --author "Knuth"
        book-memex query list --subject "Python" --limit 20
        book-memex list --favorite                # List favorites
        book-memex list --status reading          # Currently reading
        book-memex list --status completed        # Completed books
        book-memex list --view favorites          # List from a view
        book-memex list --view "programming"      # List from custom view
    """
    from .library_db import Library
...
def book_info(
    ...
    Examples:
        book-memex book info 42
        book-memex book info 42 --format json
    """
    from .library_db import Library
...
def book_status(
    ...
    Examples:
        book-memex book status 42                    # Show current status
        book-memex book status 42 --set reading      # Mark as currently reading
        book-memex book status 42 --set completed    # Mark as completed
    """
    from .library_db import Library
...
def book_progress(
    ...
    Examples:
        book-memex book progress 42                # Show current progress
        book-memex book progress 42 --set 50       # Set to 50%
    """
    from .library_db import Library
...
def book_open(
    ...
    Examples:
        book-memex book open 42                # Open first available file
        book-memex book open 42 --format pdf   # Open PDF version
    """
    import subprocess
    import platform
    from .library_db import Library
...
def book_rate(
    ...
    Example:
        book-memex book rate 42 --rating 4.5
        book-memex book rate 42 ~/my-library --rating 4.5
    """
    from .library_db import Library
...
def book_favorite(
    ...
    Examples:
        book-memex book favorite 42
        book-memex book favorite 42 --unfavorite
    """
    from .library_db import Library
...
def book_tag(
    ...
    Use 'book-memex tag' for database-wide hierarchical tag management.

    Examples:
        book-memex book tag 42 --tags "to-read,programming"
        book-memex book tag 42 --tags "to-read" --remove
    """
    from .library_db import Library
...
def book_edit(
    ...
    Examples:
        book-memex book edit 42 --title "New Title"
        book-memex book edit 42 --authors "John Doe, Jane Smith"
        book-memex book edit 42 --series "My Series" --series-index 3
        book-memex book edit 42 --add-subject "Programming"
    """
    from .library_db import Library
...
def book_merge(
    ...
    Examples:
        book-memex book merge 42 43         # Merge book 43 into book 42
        book-memex book merge 42 43,44,45   # Merge multiple books into 42
        book-memex book merge 42 43 --dry-run  # Preview merge without changes
    """
    from .library_db import Library
    from rich.table import Table
...
def book_bulk_edit(
    ...
    Examples:
        book-memex book bulk-edit --ids 1,2,3 --language en
        book-memex book bulk-edit --search "python" --add-tag "Programming"
        book-memex book bulk-edit --view favorites --rating 5
        book-memex book bulk-edit --search "author:Knuth" --add-subject "Computer Science"
    """
    from .library_db import Library
    from rich.table import Table
...
def book_similar(
    ...
    Examples:
        book-memex book similar 42
        book-memex book similar 42 --limit 20
        book-memex book similar 42 --mode metadata
    """
    from .library_db import Library
...
def book_export(
    ...
    Examples:
        book-memex book export 42                          # Print JSON to stdout
        book-memex book export 42 -f bibtex                # Print BibTeX citation
        book-memex book export 42 -o book.json             # Save JSON to file
        book-memex book export 42 -f files -o ./exports/   # Copy files to directory
        book-memex book export 42 -o ./exports/ --include-files  # JSON + files
    """
    from .library_db import Library
    import shutil
...
def purge(
    ...
    Examples:
        ...
        book-memex book purge ~/my-library --no-files

        # Preview books without supported ebook formats
        book-memex book purge ~/my-library --no-supported-formats

        # Delete books without files (after confirming)
        book-memex book purge ~/my-library --no-files --execute

        # Delete books with only unsupported formats
        book-memex book purge ~/my-library --no-supported-formats --execute

        # Delete unread books with rating <= 2 and their files
        book-memex book purge ~/my-library --unread --max-rating 2 --execute --delete-files

        # Delete all books in a specific language
        book-memex book purge ~/my-library --language fr --execute
    """
    from .library_db import Library
    from rich.table import Table
...
def note_add(
    ...
    Examples:
        book-memex note add 42 --content "Great explanation of algorithms"
        book-memex note add 42 --highlight "To be or not to be" --content "Hamlet's dilemma" --page 42
        book-memex note add 0 --content "Need a good intro to category theory" --category wishlist
    """
    from .library_db import Library
...
def note_list(
    ...
    Example:
        book-memex note list 42
        book-memex note list 42 --category important
    """
    from .library_db import Library
...
def note_extract(
    ...
    Examples:
        book-memex note extract 42                    # Extract from all formats
        book-memex note extract 42 --format pdf       # Extract only from PDF
    """
    from .library_db import Library
...
def note_export(
    ...
    Examples:
        book-memex note export 42                          # Print as markdown
        book-memex note export 42 -o notes.json -f json    # Save as JSON
    """
    from .library_db import Library
    from .services.marginalia_service import MarginaliaService
...
def export_json(
    ...
    Examples:
        book-memex export json ~/my-library ~/backup.json
        book-memex export json ~/my-library ~/favorites.json --view favorites
    """
    from .library_db import Library
    import json
...
def export_csv(
    ...
    Examples:
        book-memex export csv ~/my-library ~/books.csv
        book-memex export csv ~/my-library ~/programming.csv --view programming
    """
    from .library_db import Library
    import csv
...
def export_html(
    ...
    Examples:
        ...
        book-memex export html ~/my-library ~/library.html

        # Export a view
        book-memex export html ~/my-library ~/favorites.html --view favorites

        # Export for Hugo deployment with file copying
        book-memex export html ~/my-library ~/hugo/static/library.html \\
...
def export_opds(
    ...
    title: str = typer.Option("book-memex Library", "--title", "-t", help="Catalog title"),
    ...
    Examples:
        ...
        book-memex export opds ~/my-library ~/public/catalog.xml

        # Export a view
        book-memex export opds ~/my-library ~/public/favorites.xml --view favorites
        ...
        book-memex export opds ~/my-library ~/catalog.xml --favorite
    """
    from .library_db import Library
...
def export_goodreads(
    ...
    Examples:
        book-memex export goodreads ~/my-library ~/goodreads.csv
        book-memex export goodreads ~/my-library ~/favorites.csv --view favorites
    """
    from .library_db import Library
...
def export_calibre(
    ...
    Examples:
        book-memex export calibre ~/my-library ~/calibre.csv
        book-memex export calibre ~/my-library ~/programming.csv --view programming
    """
    from .library_db import Library
    from .services.export_service import ExportService
...
def export_echo(
    ...
    Examples:
        book-memex export echo ~/my-library ~/archive/
        book-memex export echo ~/my-library ~/archive/ --owner "John Doe"
        book-memex export echo ~/my-library ~/archive/ --view favorites --no-db
    """
    from .library_db import Library
...
def read_book(
    ...
    Examples:
        book-memex book read 42                  # Open book in default app
        book-memex book read 42 --text           # Display extracted text
        book-memex book read 42 --text --page 5  # View specific page
    """
    import subprocess
    import platform
    from .library_db import Library
...
# ============================================================================
# BOOK REVIEW COMMANDS (book-memex book review ...)
# ============================================================================
...
def review_add(
    ...
    Examples:
        book-memex book review add 42 --content "An excellent introduction to algorithms"
        book-memex book review add 42 -c "Great book!" -t "Highly Recommended" -r 5
        book-memex book review add 42 -c "Key points: ..." --type summary
    """
    from .library_db import Library
...
def review_list(
    ...
    Examples:
        book-memex book review list 42
        book-memex book review list 42 --format json
        book-memex book review list 42 --format markdown
    """
    from .library_db import Library
    from rich.table import Table
    import json
...
def review_edit(
    ...
    Examples:
        book-memex book review edit 1 --content "Updated thoughts..."
        book-memex book review edit 1 --rating 4 --title "New Title"
    """
    from .library_db import Library
...
def review_delete(
    ...
    Examples:
        book-memex book review delete 1
        book-memex book review delete 1 --yes  # Skip confirmation
    """
    from .library_db import Library
...
def serve(
    ...
    Configuration:
        ...
        Use 'book-memex config' to set default library path and server settings.
    ...
    Examples:
        ...
        book-memex serve ~/my-library --port 8080

        # Start with auto-reload (development)
        book-memex serve --reload
    """
    from book_memex.config import load_config
    import webbrowser
    ...
    try:
        ...
        console.print(f"[blue]Starting book-memex server...[/blue]")
...
def config(
    ...
    Examples:
        ...
        book-memex config --library-path ~/my-library

        # Set server to auto-open browser
        book-memex config --server-auto-open --server-host 0.0.0.0

        # Set multiple values
        book-memex config --library-path ~/my-library --server-port 9000
    """
    from book_memex.config import (
    ...
    if show or not has_settings:
        ...
        console.print(f"\n[dim]Edit with: book-memex config --library-path <path> --server-port <port> etc.[/dim]")
    ...
    if changes:
        ...
        console.print("[dim]Use 'book-memex config --show' to view current settings[/dim]")
...
def tag_list(
    ...
    Examples:
        book-memex tag list ~/my-library              - List all tags
        book-memex tag list ~/my-library -t Work      - List books tagged with "Work"
        book-memex tag list ~/my-library -t Work -s   - Include books from Work/* subtags
    """
...
def tag_tree(
    ...
    Examples:
        book-memex tag tree ~/my-library              - Show all tags as tree
        book-memex tag tree ~/my-library -r Work      - Show Work tag subtree
    """
    from book_memex.library_db import Library
    from book_memex.services.tag_service import TagService
...
def tag_add(
    ...
    Examples:
        book-memex tag add 42 Work ~/my-library
        book-memex tag add 42 Work/Project-2024 ~/my-library -d "2024 project books"
        book-memex tag add 42 Reading-List ~/my-library -c "#3498db"
    """
    from book_memex.library_db import Library
    from book_memex.services.tag_service import TagService
...
def tag_remove(
    ...
    Examples:
        book-memex tag remove 42 Work ~/my-library
        book-memex tag remove 42 Work/Project-2024 ~/my-library
    """
    from book_memex.library_db import Library
    from book_memex.services.tag_service import TagService
    from book_memex.db.models import Book
...
def tag_rename(
    ...
    Examples:
        book-memex tag rename Work Archive ~/my-library
        book-memex tag rename Work/Old Work/Completed ~/my-library
    """
    from book_memex.library_db import Library
    from book_memex.services.tag_service import TagService
...
def tag_delete(
    ...
    Examples:
        book-memex tag delete OldTag ~/my-library
        book-memex tag delete OldProject ~/my-library -r     - Delete with children
        book-memex tag delete Archive ~/my-library -r -f     - Delete without confirmation
    """
    from book_memex.library_db import Library
    from book_memex.services.tag_service import TagService
...
def tag_stats(
    ...
    Examples:
        book-memex tag stats ~/my-library              - Overall tag statistics
        book-memex tag stats ~/my-library -t Work      - Stats for specific tag
    """
    from book_memex.library_db import Library
    from book_memex.services.tag_service import TagService
    ...
    try:
        ...
        if tag_path:
            ...
            # Count tagged books
            from book_memex.db.models import book_tags
...
def queue_list(
    ...
    Examples:
        book-memex queue list
        book-memex queue list ~/my-library
    """
    from .library_db import Library
    ...
    try:
        ...
        if not queue:
            ...
            console.print("Add books with: book-memex queue add <book_id>")
            lib.close()
            return
...
def queue_add(
    ...
    Examples:
        book-memex queue add 42                    # Add to end of queue
        book-memex queue add 42 --position 1       # Add to top of queue
        book-memex queue add 42 -p 3               # Add at position 3
    """
    from .library_db import Library
...
def queue_move(
    ...
    Examples:
        book-memex queue move 42 1         # Move to top
        book-memex queue move 42 3         # Move to position 3
    """
    from .library_db import Library
...
def queue_clear(
    ...
    Examples:
        book-memex queue clear
        book-memex queue clear --yes
    """
    from .library_db import Library
...
def view_create(
    ...
    Examples:
        book-memex view create favorites --favorite
        book-memex view create programming --subject programming
        book-memex view create top-rated --rating-gte 4 --description "My best books"
        book-memex view create complex --from-file my-view.yaml
        book-memex view create english --sql "SELECT id FROM books WHERE language = 'en'"
    """
    from .library_db import Library
    from .views import ViewService
...
def view_list(
    ...
    Examples:
        book-memex view list
        book-memex view list --no-builtin
    """
    from .library_db import Library
    from .views import ViewService
...
def view_show(
    ...
    Examples:
        book-memex view show favorites
        book-memex view show programming --limit 50
        book-memex view show my-view --definition
    """
    from .library_db import Library
    from .views import ViewService
...
def view_delete(
    ...
    Examples:
        book-memex view delete old-view
        book-memex view delete old-view --force
    """
    from .library_db import Library
    from .views import ViewService
...
def view_add(
    ...
    Examples:
        book-memex view add favorites 42
        book-memex view add my-collection 17
    """
    from .library_db import Library
    from .views import ViewService
...
def view_remove(
    ...
    Examples:
        book-memex view remove favorites 42
    """
    from .library_db import Library
    from .views import ViewService
...
def view_set(
    ...
    Examples:
        book-memex view set my-view 42 --title "Better Title"
        book-memex view set my-view 42 --description "My notes about this book"
        book-memex view set my-view 42 --position 1
    """
    from .library_db import Library
    from .views import ViewService
...
def view_unset(
    ...
    Examples:
        book-memex view unset my-view 42                    # Remove all overrides
        book-memex view unset my-view 42 --field title     # Remove only title override
    """
    from .library_db import Library
    from .views import ViewService
...
def view_export(
    ...
    Examples:
        book-memex view export my-view                     # Print to stdout
        book-memex view export my-view -o my-view.yaml    # Save to file
    """
    from .library_db import Library
    from .views import ViewService
...
def view_import(
    ...
    Examples:
        book-memex view import my-view.yaml
        book-memex view import my-view.yaml --overwrite
    """
    from .library_db import Library
    from .views import ViewService
...
def mcp_serve(
    ...
    try:
        from book_memex.mcp.server import run_server
    except ImportError:
        console.print("[red]MCP dependencies not installed. Run: pip install book-memex[mcp][/red]")
        raise typer.Exit(code=1)
